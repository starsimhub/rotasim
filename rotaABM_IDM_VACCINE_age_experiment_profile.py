...
immunity_hypothesis = int(sys.argv[1])
reassortment_rate = float(sys.argv[2])
fitness_hypothesis = int(sys.argv[3])
vaccine_hypothesis = int(sys.argv[4])
experimentNumber = int(sys.argv[5])
...
class host(object): ## host class
    ...
    def recover(self,strainCounts):
        ...
        for path in self.infecting_pathogen:
            if not path.is_reassortant:
                ...
                self.immunity.append((path.strain, t))
    ...
    def can_variant_infect_host(self, strainIn, currentInfections):
        ...
        immune_strains = set([s[0] for s in self.immunity])
        ...
        if immunity_hypothesis == 1:
            ...
            immune_strains = [s[:numAgSegments] for s in immune_strains]
            ...
            for i in range(numAgSegments):         
                immune_genotypes = [strain[i] for strain in immune_strains]
                if strainIn[i] in immune_genotypes:
                    return False
            return True
            ...
            shared_genotype = False      
            immune_ptypes = [strain[1] for strain in immune_strains]
            ...
            # Partial heterotypic immunity
            shared_genotype = False      
            immune_gtypes = [strain[0] for strain in immune_strains]
            if strainIn[0] in immune_gtypes:
                return False
            else:
                return True
        else:
...
def get_weights_by_age(host_pop):
    bday_array = np.array([x.bday for x in host_pop])
    weights = t - bday_array
...
def death_event(num_deaths, infected_pop, host_pop, strainCount):
    ...
    weights = get_weights_by_age(host_pop)
    dying_hosts_indices = np.random.choice(range(len(host_pop)), p=weights, size=num_deaths, replace=False)
    dying_hosts = [host_pop[i] for i in dying_hosts_indices]

    # Initialize counters for batch updates
    cases_to_deduct = 0
    immunity_to_deduct = 0
    strain_deductions = defaultdict(int)

    # Process dying hosts
    for h in dying_hosts:
        if h.isInfected():
            cases_to_deduct += 1
            if h in infected_pop:
                infected_pop.remove(h)
            for path in h.infecting_pathogen:
                if not path.is_reassortant:
                    strain_deductions[path.strain] += 1
        if h.isImmune():
            immunity_to_deduct += 1

    # Apply batch updates
    cases -= cases_to_deduct
    immunityCounts -= immunity_to_deduct
    for strain, deduction in strain_deductions.items():
        strainCount[strain] -= deduction
    host_pop = [h for h in host_pop if h not in dying_hosts]

def recovery_event(num_recovered, infected_pop, strainCount):
    global cases, immunityCounts

    # Ensure there's at least one infection older than 0
    if not any(x.get_oldest_current_infection() > 0 for x in infected_pop):
        return

    weights = np.array([x.get_oldest_current_infection() for x in infected_pop])
    weights /= weights.sum()  # Normalize weights

    # Select hosts for recovery based on weighted probabilities
    ...
    # Efficiently remove recovering hosts from infected_pop
    """ infected_pop = [host for host in infected_pop if host not in recovering_hosts] """
...
def waning_event(host_pop, wanings):
    ...
    precomputed_values = [(h, h.get_oldest_infection()) for h in host_pop if h.isImmune()]
    hosts_with_immunity = sorted(precomputed_values, key=lambda x: x[1], reverse=True)

    # Directly update the hosts without looping through each one
    for h,_ in hosts_with_immunity[:wanings]:
        h.immunity = []
        h.immunityCount = 0

    # Update immunityCounts in one operation
    immunityCounts -= min(len(hosts_with_immunity), wanings)
...
initialSegmentCombinations = [(1,1,1,1), (2,2,1,1), (3,3,1,1), (4,4,1,1)] #[(1,1,1,1), (2,2,1,1), (1,2,1,1), (2,1,1,1)]
