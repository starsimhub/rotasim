"""
Test calibration with cross-protection parameters instead of maternal immunity

Strategy:
- Remove maternal immunity (set to 0)
- Calibrate cross-protection efficacies (homotypic, partial heterotypic, complete heterotypic)
- Lower cross-protection → more reinfections in children → younger age distribution
"""
import sciris as sc
...
print("="*60)
print("Testing Cross-Protection Calibration")
print("="*60)
print("\nStrategy:")
print("  1. Remove maternal immunity (makes distribution older)")
print("  2. Calibrate cross-protection efficacies")
print("  3. Lower cross-protection → more child reinfections → younger distribution")
print("="*60)

# Create sim with parameters from tests/simple.py
sim = rs.Sim(
    n_agents=5000,
    start='2000-01-01',
    stop='2010-01-01',
...
# CROSS-PROTECTION CALIBRATION STRATEGY:
# 1. reporting_rate: Scale overall incidence magnitude (~0.02%)
# 2. homotypic_immunity_efficacy: Protection from same strain (currently 90%)
# 3. partial_heterotypic_immunity_efficacy: Protection from shared G or P (currently 50%)
# 4. complete_heterotypic_immunity_efficacy: Protection from different strains (currently 30%)
# 5. rel_beta: Adjust overall transmission intensity
# 6. Remove maternal immunity (set to 0)

# Lower cross-protection values allow more reinfections
# Children have fewer prior infections → benefit less from cross-protection
# Adults have many prior infections → still have high immunity despite lower cross-protection
# Result: More reinfections in children → younger age distribution

calib_pars = sc.objdict(
    reporting_rate=[0.0002, 0.0001, 0.001],  # 0.01-0.1% surveillance capture
    homotypic_immunity_efficacy=[0.5, 0.1, 0.9],  # 10-90% (down from 90% fixed)
    partial_heterotypic_immunity_efficacy=[0.2, 0.0, 0.5],  # 0-50% (down from 50% fixed)
    complete_heterotypic_immunity_efficacy=[0.1, 0.0, 0.3],  # 0-30% (down from 30% fixed)
    rel_beta=[2.0, 1.0, 3.0],  # 1-3x transmission
    maternal_immunity_efficacy=[0.0, 0.0, 0.0],  # Remove maternal immunity (makes distribution older)
)
