"""
Test to verify the incidence calculation bug fix

This test demonstrates that:
1. The new code (with age_counts) produces reasonable incidence values
2. The old code (without age_counts) produces inflated values
3. The age-specific population counts are extracted correctly
"""
import sys
...
from calibrate_uk import extract_age_specific_population_counts
import process_incidence_uk

print("=" * 80)
print("TESTING INCIDENCE CALCULATION BUG FIX")
print("=" * 80)

# Use realistic parameters from recent calibration
test_pars = {
    'base_beta': 0.263,
...
if is_reasonable and old_was_inflated:
    ...
    print(f"  - This may indicate other calibration issues")
    print(f"  - Bug fix itself is correct, but model parameters may need adjustment")
