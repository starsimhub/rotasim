"""
Exp 06 — infection-number-only symptoms, matched to exp 05 (see README.md).

Same setup as exp 05 (homogeneous mixing via RandomNet, strong Erlang maternal) but
symptom_model='infection_number'. Forward-runs p_symp sets x base_beta and checks
whether the symptomatic-by-age profile reproduces the low-<6m / 6-11mo-peak /
declining shape that the peaked age curve achieved. No calibration.
...
def main():
    ...
    sim_config = dict(
        ...
        symptom_model='infection_number', maternal_n_stages=cfg['maternal_n_stages'],
    )

    combos = list(itertools.product(cfg['p_symp_sets'].items(), cfg['base_beta_sweep']))
    rng = np.random.default_rng(cfg['seed'])
    seeds = rng.integers(0, 1_000_000, len(combos)).tolist()
    tasks, labels = [], []
    for ((name, ps), bb), s in zip(combos, seeds):
        pars = dict(base_beta=bb, p_symp_1=ps[0], p_symp_2=ps[1], p_symp_3plus=ps[2],
    ...
    print(f"Forward-running {len(combos)} (p_symp-set x base_beta) combos, {cfg['n_agents']} agents each.")
    ...
    with jsonl.open('w') as f:
        for (name, bb), mo in zip(labels, outs):
            ...
            r = dict(p_symp_set=name, base_beta=bb, symp_ir=ir,
                     first_med=float(mo['first_infection']['median']))
            rows.append(r); f.write(json.dumps(r) + '\n')

    print("\n=== symptomatic IR by age (infection-number) — peak+decline like exp 05? ===")
    print(f"  {'DATA':<24} {[round(v,2) for v in tgt_ir]}")
    any_peak = False
    for r in rows:
        ir = np.array(r['symp_ir'])
        peaked = (ir[1] > ir[0]) and (ir[1] > ir[2]) and (ir[2] > ir[3])
        any_peak |= peaked
        print(f"  {r['p_symp_set']:<12} beta={r['base_beta']:.2f}: {[round(v,2) for v in r['symp_ir']]}  "
              f"first-med={r['first_med']:.1f} {'<-- PEAK+decline' if peaked else ''}")
    print(f"\n  RESULT: {'infection-number ALSO reproduces the peak shape' if any_peak else 'infection-number does NOT reproduce the peak+decline shape (age severity needed)'}")

    fig, ax = plt.subplots(figsize=(10, 6))
    x = np.arange(len(BINS))
    ax.plot(x, tgt_ir, 'r*-', ms=16, lw=2.5, label='MAL-ED data', zorder=5)
    for r in rows:
        ax.plot(x, r['symp_ir'], 'o-', alpha=0.7, label=f"{r['p_symp_set']}, beta={r['base_beta']}")
    ax.set_xticks(x); ax.set_xticklabels(BINS); ax.set_ylabel('Symptomatic IR /100 PM')
    ax.set_title('Exp 06 — infection-number-only symptoms (homogeneous mixing, strong maternal)\n'
                 'does it reproduce the shape exp 05 achieved? (data in red)')
    ax.legend(frameon=False, fontsize=8); ax.spines[['top', 'right']].set_visible(False)
    fig.tight_layout()
    fig.savefig(out_root / 'figures' / 'infnum_shape.png', dpi=150)
    print(f"\nwrote {out_root / 'figures' / 'infnum_shape.png'}")
