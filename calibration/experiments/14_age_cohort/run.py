"""
Exp 14 — analysis: age-symptom + titer maternal under the MAL-ED cohort observation.
The Set-2 age partner; overlays exp 13 (infnum cohort) to show the matched pair.
...
DB = CALIB_DIR / 'rota_maled_bangladesh_titer_cohort.db'
STUDY = 'rota_maled_bangladesh_titer_cohort'
INFNUM_COHORT_EXP13 = [2.88, 4.10, 2.59, 2.14]   # exp-13 infnum+titer cohort best (partner)


def main():
    targets = P.load_targets('bangladesh')
    tir = targets['ir_by_age']; tgt_ir = tir['IR'].values; tgt_p = tgt_ir / tgt_ir.sum()
    ...
    print(f"\n  TARGET IR {[round(v,2) for v in tgt_ir]}  KM first-detect {km['median']:.2f}mo  repeat {rf['frac']:.3f}")
    print(f"  age+titer COHORT best (trial {bt.number}): objective {bt.value:.2f}")
    print(f"    IR {[round(v,2) for v in med]}  peak {BINS[int(np.argmax(med))]}  cosine {cosine:.4f}")
    print(f"    <6m {med[0]:.2f} (target 1.91) -- crushed like exp 12, or recovered?")
    print(f"    KM first-detect {med_first:.2f}mo (target {km['median']:.2f}); repeat {med_rep:.3f} (target {rf['frac']:.3f})")
    print(f"    base_beta {bt.params.get('base_beta'):.3f}  titer median {bt.params.get('maternal_titer_median'):.1f} "
          ...
          f"eff {bt.params.get('maternal_immunity_efficacy'):.2f}")
    verdict = "GOOD fit -> Set 2 matched pair works" if cosine > 0.97 and med[0] > 1.0 else "POOR (titer over-suppresses age <6m) -> use {age+Erlang} vs {infnum+titer}"
    print(f"  VERDICT: {verdict}")

    x = np.arange(len(BINS))
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(x, tgt_ir, 'r*-', ms=18, lw=2.5, label='MAL-ED data', zorder=6)
    ax.plot(x, INFNUM_COHORT_EXP13, 'o--', color='#0571b0', alpha=0.5, lw=1.5,
            label='infnum+titer cohort (exp 13)', zorder=3)
    ax.plot(x, med, 'o-', color='#d95f02', lw=2, label='age+titer cohort (exp 14)', zorder=4)
    ...
    ax.set_title(f'Exp 14 — age+titer under cohort obs (cosine {cosine:.3f}; <6m {med[0]:.2f} vs 1.91)\n'
                 'Set-2 matched pair with exp 13 (infnum+titer cohort)')
    ax.legend(frameon=False); ax.spines[['top', 'right']].set_visible(False)
    fig.tight_layout()
    (HERE / 'figures').mkdir(parents=True, exist_ok=True)
    (HERE / 'outputs').mkdir(parents=True, exist_ok=True)
    fig.savefig(HERE / 'figures' / 'age_cohort_fit.png', dpi=150)
    ...
    print(f"\nwrote {HERE/'figures'/'age_cohort_fit.png'} + scorecard.json")
