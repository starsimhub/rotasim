"""Generate confidence intervals for infection_number MLE model with multiple stochastic runs"""
import sys
...
# MLE parameters from calibration (infection_number model)
MLE_PARAMS = {
    'reporting_rate': 0.0006545677784196243,
    'base_beta': 1.735290653804337
}
...
print("=" * 60)
print("MLE Confidence Intervals: infection_number")
print("=" * 60)
print(f"\nMLE Parameters:")
...
for run in range(N_RUNS):
    ...
    # Create analyzer with infection-based severity (default behavior)
    analyzer = rs.InfectedStrainStats(
        use_infection_based_severity=True  # Uses infection number to determine severity
    )
    ...
    calib = UKAgeCalibration(
        ...
        total_trials=1,
        debug=False,
        symptom_model='infection_number',  # Use infection_number model
    )
...
# Save results
results_dict = {
    'model': 'infection_number',
    'severity_type': 'infection_based',
...
results_file = thisdir / 'mle_confidence_intervals_infection_number.json'
