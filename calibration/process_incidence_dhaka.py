...
def process_data(filename=None, sheet=None):
    ...
    """
    if filename is None:
        filename = thisdir / 'CalibrationDatafile_prevax_Dhaka.xlsx'
    if sheet is None:
        sheet = 'Dhaka_ageincidence'
...
def process_model(dat=None, popsize=None, verbose=False):
    ...
    # Subset to years 1-7 for now
    initial7 = dat[(dat['CollectionTime'] < 8) & (dat['CollectionTime'] > 1)]

    if verbose: print(initial7['Strain'].value_counts())
    initial7['Strain3'] = 'Other'
    initial7.loc[initial7['Strain'] == 'G1P8A1B1', 'Strain3'] = 'G1P8'
    initial7.loc[initial7['Strain'] == 'G2P4A1B1', 'Strain3'] = 'G2P4'
    initial7.loc[initial7['Strain'] == 'G9P8A1B1', 'Strain3'] = 'G9P8'

    GenoDist = initial7['Strain3'].value_counts().reset_index()
    ...
    initial7['AgeCat'] = np.nan
    initial7.loc[initial7['Age'].isin(['0-2', '2-4', '4-6', '6-12']), 'AgeCat'] = '<1 y'
    initial7.loc[initial7['Age'].isin(['12-24']), 'AgeCat'] = '1-2 y'
    initial7.loc[initial7['Age'].isin(['24-36', '36-48', '48-60']), 'AgeCat'] = '2-5 y'
    initial7.loc[initial7['Age'] == '60+', 'AgeCat'] = '>=5 y'

    # Now, getting cases by age bin
    cases_summary = initial7.groupby(['AgeCat', 'CollectionTime']).agg(Cases_age=('id', 'nunique')).reset_index()
    ...
    pop_pooled = initial7.groupby(['CollectionTime', 'AgeCat']).apply(lambda x: x.head(1)).reset_index(drop=True)
    ...
    pop_pooled2.loc[pop_pooled2['AgeCat'] == '<1 y', 'Pop_Age'] = pop_pooled2['PopulationSize'] * 0.026
    pop_pooled2.loc[pop_pooled2['AgeCat'] == '1-2 y', 'Pop_Age'] = pop_pooled2['PopulationSize'] * 0.026
    pop_pooled2.loc[pop_pooled2['AgeCat'] == '2-5 y', 'Pop_Age'] = pop_pooled2['PopulationSize'] * 0.079
    pop_pooled2.loc[pop_pooled2['AgeCat'] == '>=5 y', 'Pop_Age'] = pop_pooled2['PopulationSize'] * 0.869
