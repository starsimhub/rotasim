...
class TestRotaVaccinationBasic:
    ...
    def test_vaccination_initialization(self):
        ...
        vax = rs.RotaVaccination(
            start_date="2025-01-01", G_antigens=[1, 2], P_antigens=[8, 4], n_doses=2, dose_effectiveness=[0.6, 0.8]
        )
...
class TestRotaVaccinationSimulation:
    ...
    def create_test_sim(self, vax_kwargs=None, sim_kwargs=None):
        ...
        default_vax = {"start_date": "2020-01-01", "verbose": False}
    ...
    def test_vaccination_coverage_identification(self):
        ...
        # Test G1P8 vaccine
        sim = self.create_test_sim({"G_antigens": [1], "P_antigens": [8]})
        sim.init()
        vax = sim.interventions[0]
        ...
        sim2 = self.create_test_sim({"G_antigens": [1, 2], "P_antigens": [8, 4]})
        sim2.init()
    ...
    def test_age_eligibility(self):
        ...
        sim = self.create_test_sim({"min_age": ss.days(60), "max_age": ss.days(300)})  # ~2 months  # ~10 months
        sim.init()
        vax = sim.interventions[0]
    ...
    def test_vaccination_timing(self):
        ...
        sim = self.create_test_sim({"start_date": "2021-01-01"})  # Start after sim start
        sim.init()
        vax = sim.interventions[0]
        ...
        sim2 = self.create_test_sim({"start_date": "2020-01-01", "end_date": "2020-06-01"})
        sim2.init()
        vax2 = sim2.interventions[0]
    ...
    def test_protection_waning(self):
        ...
        for disease in vax.covered_diseases:
            ...
            if np.any(vaccinated_mask):
                ...
                assert np.any(
                    current_rel_sus[vaccinated_mask] >= mid_values[vaccinated_mask]
                ), f"No waning detected for {disease.name}"
...
class TestRotaVaccinationSummary:
    ...
    def create_test_sim(self):
        """Helper to create test simulation"""
        vax = rs.RotaVaccination(
            start_date="2020-01-01", uptake_dist=ss.bernoulli(0.5), verbose=False  # Moderate uptake
        )
...
class TestRotaVaccinationCrossProtection:
    ...
    def test_strain_matching_functions(self):
        ...
        vax = rs.RotaVaccination(start_date="2025-01-01", G_antigens=[1, 2], P_antigens=[8, 4])
    ...
    def test_cross_protection_effectiveness_calculation(self):
        ...
        for disease in vax.covered_diseases:
            ...
            assert (
                abs(match_efficacy - expected_efficacy) < 1e-6
            ), f"Disease {disease.name} (G{disease.G}P{disease.P}): expected {expected_efficacy}, got {match_efficacy}"
    ...
    def test_precomputed_match_efficacies(self):
        ...
        for disease in vax.covered_diseases:
            ...
            assert (
                precomputed == manual_calculation
            ), f"Mismatch for {disease.name}: precomputed={precomputed}, manual={manual_calculation}"
