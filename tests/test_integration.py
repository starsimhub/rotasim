...
def test_multi_strain_creation():
    ...
    initial_strains_large = [(1, 8), (2, 4), (3, 6), (4, 8), (9, 8), (12, 8), (9, 6), (11, 4)]
    sim_large = rs.Sim(
        scenario={
...
def test_dormant_strain_handling():
    ...
    print(
        f"[OK] Dormant strain handling: {len(summary['active_strains'])} active, {len(summary['dormant_strains'])} dormant"
    )
...
def test_fitness_scenarios():
    ...
    sim_custom = Sim(
        scenario={
            "strains": {
                strain: {"fitness": custom_fitness.get(strain, 1.0), "prevalence": 0.01} for strain in initial_strains
            },
            "default_fitness": 1.0,
        },
        verbose=False,
