...
def test_small_simulation_run():
    ...
    if len(proportion_cols) > 0:
        ...
        if len(non_zero_sums) > 0:
            print(
                f"   Row sums (non-zero): min={non_zero_sums.min():.3f}, max={non_zero_sums.max():.3f}, mean={non_zero_sums.mean():.3f}"
            )
    ...
    if len(count_cols) > 0:
        ...
        print(
            f"   Total counts: min={total_counts.min():.0f}, max={total_counts.max():.0f}, mean={total_counts.mean():.1f}"
        )
...
def test_multiple_strain_scenarios():
    ...
    for name, strains in scenarios:
        ...
        print(
            f"   Analyzer columns: {len(strain_cols)} total ({len(proportion_cols)} proportion + {len(count_cols)} count)"
        )
