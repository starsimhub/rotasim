...
def test_reassortment_with_rotasim():
    ...
    sim_no_connectors = Sim(scenario="simple", connectors=[])
    ...
    sim_custom = Sim(scenario="simple", connectors=[custom_reassortment])
...
def test_reassortment_analysis():
    ...
    print(
        f"   Actual: {summary['total_diseases']} total ({len(summary['active_strains'])} active + {len(summary['dormant_strains'])} dormant)"
    )

    if (
        summary["total_diseases"] == expected_total
        and len(summary["active_strains"]) == expected_active
        and len(summary["dormant_strains"]) == expected_dormant
    ):
