...
# df = df[(df['CollectionTime'] >= 15) & (df['CollectionTime'] < 20)]
# filtered = genotype_counts_with_total[(15 <= genotype_counts_with_total['Time']) & (genotype_counts_with_total['Time'] < 20)]
...
df['strain_type'] = df['Strain'].apply(lambda x: get_vaccine_match(vaccine_strain, x))

def get_severity_below_age_with_vac_group(df, l, u, age_groups, vac_group):
    severe_incidence = df[(df['Time'] >= l) & (df['Time'] < u) & (df['Severity'] == True) & (df['Age'].isin(age_groups)) & (df['strain_type'] == vac_group)].copy()
...
for age_group in [all_ages, below_5, below_2]:
    df2 = df[df['Age'].isin(age_group)]
    genotype_counts = df2.groupby(['strain_type', 'Time']).size().reset_index(name='Counts')
    ...
    filtered = genotype_counts_with_total[(15 <= genotype_counts_with_total['Time']) & (genotype_counts_with_total['Time'] < 20)]
    avgs = filtered.groupby('strain_type').agg({'Percentage':'mean'}).reset_index()

    print("================ Average percentage for age group:", age_group, 15, 20, "================")
    print(avgs)

    filtered = genotype_counts_with_total[(35 <= genotype_counts_with_total['Time']) & (genotype_counts_with_total['Time'] < 40)]
    avgs = filtered.groupby('strain_type').agg({'Percentage':'mean'}).reset_index()

    print("================ Average percentage for age group:", age_group, 35, 40, "================")
    print(avgs)

    fig = px.area(genotype_counts_with_total, x="Time", y="Percentage", color="strain_type", line_group="strain_type")
    """ output_file_age = output_file.replace(".jpg", "_vaccine_" + ".jpg") """
    fig.update_layout(
        font_size=18,
        xaxis_title="Year since simulation started",
        yaxis_title="Percentage",
        title="Percentage Based on Vaccine Efficacy: " + experiment_siffix
    )
    fig.show()